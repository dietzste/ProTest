...
If (r_Main1 = 1) OR (r_Main3 = 1)
	...
	if (c_SkipLastPart = 1)
		...
		if (TimeOutMsgSkippedIntro > 0)
			MsgBox, 4096, Intro �bersprungen! , Intro �bersprungen! (No match for fn: "%fnOCR%"), %TimeOutMsgSkippedIntro%
	...
	Loop, parse, LFDListTempFile, `n, `r
			...
			if (Instr(A_LoopField, "LFD_"))
				...
				if (CurrentTempFileDigits != CurrentLFDDigits)
					...
					%A_Space%zur aktuellen Studie. Der aktuelle Durchlauf wird deshalb beendet. Bitte Projektdatei �ber das F10 Men� �ndern!
					)
...
If (LFDSections != "ERROR")
	...
	If (LFDDigitsCount > 1)
		...
		InputBoxText := "Im TempFile starten LFDs mit unterschiedlichen Zahlen:`n" . LFDTypeList . "`nMit welchen zwei Zahlen beginnen die LFDs, die gel�scht werden sollen? (Schleife " . A_Index . "/" . LFDDigitsCount-1 . ")"
		ShowThisInputBox:
		...
		if (ErrorLevel = 1) or (ErrorLevel = 0 And CleanupDigit = "")
			...
			%A_Space%Das TempFile enth�lt weiterhin fehlerhafte Eintragungen.
			)
			...
			MsgBox, 4096, Bereinigung durchgef�hrt!, TempFile erfolgreich bereinigt! Gel�schte LFDs:`n%DeletedLFDs%
